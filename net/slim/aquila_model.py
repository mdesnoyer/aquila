        ...
        with scopes.arg_scope([ops.conv2d, ops.fc, ops.batch_norm, ops.dropout],
            ...
            with scopes.arg_scope([ops.conv2d, ops.max_pool, ops.avg_pool],
                ...
                end_points['conv0'] = ops.conv2d(inputs, 32, [3, 3], stride=2,
                                                 scope='conv0')
                # 149 x 149 x 32
                end_points['conv1'] = ops.conv2d(end_points['conv0'], 32,
                                                 [3, 3], scope='conv1')
                # 147 x 147 x 32
                end_points['conv2'] = ops.conv2d(end_points['conv1'], 64,
                                                 [3, 3], padding='SAME',
                                                 scope='conv2')
                # 147 x 147 x 64
                ...
                # 73 x 73 x 64
                # 73 x 73 x 64
                end_points['conv3'] = ops.conv2d(end_points['pool1'], 80,
                                                 [1, 1], scope='conv3')
                # 71 x 71 x 80.
                end_points['conv4'] = ops.conv2d(end_points['conv3'], 192,
                                                 [3, 3], scope='conv4')
                # 69 x 69 x 192.
            ...
            with scopes.arg_scope([ops.conv2d, ops.max_pool, ops.avg_pool],
                ...
                with tf.variable_scope('mixed_17x17x768a'):
                    with tf.variable_scope('branch3x3'):
                        branch3x3 = ops.conv2d(net, 384, [3, 3], stride=2,
                                               padding='VALID')
                    with tf.variable_scope('branch3x3dbl'):
                        branch3x3dbl = ops.conv2d(net, 64, [1, 1])
                        branch3x3dbl = ops.conv2d(branch3x3dbl, 96, [3, 3])
                    ...
                    with tf.variable_scope('branch_pool'):
                        branch_pool = ops.max_pool(net, [3, 3], stride=2,
                                                   padding='VALID')
                ...
                with tf.variable_scope('mixed_17x17x768b'):
                    ...
                    net = tf.concat(3, [branch1x1, branch7x7, branch7x7dbl,
                                        branch_pool])
                ...
                with tf.variable_scope('aux_logits'):
                    ...
                    aux_logits = ops.conv2d(aux_logits, 768, shape[1:3],
                                            stddev=0.01, padding='VALID')
                    aux_logits = ops.flatten(aux_logits)
                    aux_logits = ops.fc(aux_logits, num_abs_features,
                                        activation=None, stddev=0.001,
                                        restore=restore_logits)
                    aux_logits = ops.fc(aux_logits, 1, activation=None,
                ...
                with tf.variable_scope('mixed_8x8x2048a'):
                    ...
                    with tf.variable_scope('branch3x3'):
                        ...
                        branch3x3 = tf.concat(3, [ops.conv2d(branch3x3, 384,
                                                             [1, 3]),
                    ...
                    with tf.variable_scope('branch3x3dbl'):
                        ...
                        branch3x3dbl = tf.concat(3, [ops.conv2d(branch3x3dbl,
                                                                384, [1, 3]),
                    ...
                    with tf.variable_scope('branch_pool'):
                        branch_pool = ops.avg_pool(net, [3, 3])
                        branch_pool = ops.conv2d(branch_pool, 192, [1, 1])
                    net = tf.concat(3, [branch1x1, branch3x3, branch3x3dbl,
                                        branch_pool])
                ...
                with tf.variable_scope('logits'):
                    shape = net.get_shape()
                    net = ops.avg_pool(net, shape[1:3], padding='VALID',
                                       scope='pool')
                    # 1 x 1 x 2048
                    if dropout_keep_prob < 1.0:
                        net = ops.dropout(net, dropout_keep_prob,
                                          scope='dropout')
                    net = ops.flatten(net, scope='flatten')
                    # 2048
                    abstract_feats = ops.fc(net, num_abs_features,
                                            activation=None,
                                            scope='abst_feats',
                                            restore=restore_logits)
                    # 1024
                    logits = ops.fc(abstract_feats, 1, activation=None,
